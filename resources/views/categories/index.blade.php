                                    ...
                                    <tbody>
                                        @foreach ($categories as $ctr)
                                            <tr>
                                                ...
                                                <td class="text-right">{{ $ctr->nama_kategori }}</td>
                                                <td class="text-right">{{ $ctr->deskripsi }}</td>
                                                <td>
                                                    <div class="d-flex justify-content-center" style="gap: .5rem">
                                                        {{-- <a href="{{ url("/categories/{$ctr->id}/show") }}"
                                                            class="btn btn-info" data-toggle="tooltip" title="Detail">
                                                            <i class="fas fa-info-circle"></i>
                                                        </a> --}}
                                                        @if (Auth::user()->role ==  'admin')
                                                            <a href="{{ url("/categories/{$ctr->id}/edit") }}"
                                                                class="btn btn-primary" data-toggle="tooltip"
                                                                title="Ubah">
                                                                <i class="fas fa-pencil"></i>
                                                            </a>
                                                            <form action="{{ url("/categories/{$ctr->id}") }}"
